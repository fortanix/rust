...
if [ ! -f config.toml ]; then
    ...
    ./configure --enable-lld --set profile=compiler \
        --set install.prefix="/home/yuxiangcao/main_ws/toolchain/rust/rustup/toolchains/dev_snmalloc-2024-04-03-x86_64-unknown-linux-gnu" \
        --set install.sysconfdir="etc" \
        --set build.extended=true \
        --set build.tools="cargo,rustfmt,src" \
