use fortanix_vme_abi::{self, Addr, Request, Response};
...
use crate::sync::{Arc, OnceLock, RwLock};
use crate::sys::cvt;
...
impl From<vsock::Error> for io::Error {
    fn from(err: vsock::Error) -> io::Error {
        match err {
            vsock::Error::EntropyError => {
                io::Error::new(ErrorKind::Other, VsockErrorWrapper::from(err))
            }
            vsock::Error::ReservedPort => {
                io::Error::new(ErrorKind::InvalidInput, VsockErrorWrapper::from(err))
            }
            vsock::Error::SystemError(errno) => io::Error::from_raw_os_error(errno),
            vsock::Error::WrongAddressType => {
                io::Error::new(ErrorKind::InvalidInput, VsockErrorWrapper::from(err))
            }
            vsock::Error::ZeroDurationTimeout => {
                io::Error::new(ErrorKind::InvalidInput, VsockErrorWrapper::from(err))
            }
        }
    }
...
impl From<fortanix_vme_abi::ErrorKind> for io::ErrorKind {
    fn from(kind: fortanix_vme_abi::ErrorKind) -> io::ErrorKind {
        match kind {
            ...
            //#[unstable(feature = "io_error_more", issue = "86442")]
            fortanix_vme_abi::ErrorKind::StaleNetworkFileHandle => {
            ...
            //#[unstable(feature = "io_error_more", issue = "86442")]
            fortanix_vme_abi::ErrorKind::FilesystemQuotaExceeded => io::ErrorKind::QuotaExceeded,
            //#[unstable(feature = "io_error_more", issue = "86442")]
...
impl ConnectionInfo {
    pub(crate) fn new_stream_info(enclave_port: u32, local: Addr, peer: Addr) -> Self {
        ConnectionInfo::Stream { enclave_port, local, peer }
    }

    pub(crate) fn new_listener_info(local: Addr, enclave_port: u32) -> Self {
        ConnectionInfo::Listener { local, enclave_port }
    }
...
impl Client {
    // FIXME: Use the FNV crate for the Fowler–Noll–Vo hash function for better performance (requires upstream changes).
    fn connection_info_map() -> &'static RwLock<HashMap<RawFd, Arc<ConnectionGuard>>> {
        static CONNECTION_INFO: OnceLock<RwLock<HashMap<RawFd, Arc<ConnectionGuard>>>> =
            OnceLock::new();
    ...
    pub(crate) fn store_connection_info<FD: AsRawFd>(fd: &FD, info: ConnectionInfo) {
        ...
        if let Some(_prev) = map.insert(raw_fd, Arc::new(ConnectionGuard(info))) {
            eprintln!(
                "panic! Already keeping track of Connection info related to file descriptor {}",
                raw_fd
            );
        }
    }
    ...
    pub fn duplicate_fd(fd: RawFd) -> io::Result<RawFd> {
        let mut map = Self::connection_info_map().write().expect("ConnectionInfo RwLock poisoned");
        let info = map.get(&fd).ok_or(io::Error::from(io::ErrorKind::InvalidData))?.clone();
    ...
    fn connect(port: u32) -> Result<VsockStream<Fortanixvme>, io::Error> {
        ...
        VsockStream::connect_with_cid_port(vsock::VMADDR_CID_HOST, port).or_else(|e0| {
    ...
    pub fn bind_socket(&mut self, addr: String) -> Result<FileDesc, io::Error> {
        ...
        // specified port
        let bind = Request::Bind { addr, enclave_port };
        self.send(&bind)?;
    ...
    pub fn accept(&mut self, vsock_port: u32) -> Result<(Addr, Addr, u32), io::Error> {
        let accept = Request::Accept { enclave_port: vsock_port };
        self.send(&accept)?;
    ...
    fn close_connection(enclave_port: u32) -> Result<(), io::Error> {
        ...
        let close = Request::Close { enclave_port };
        client.send(&close)?;
    ...
    pub(crate) fn exit(code: i32) -> ! {
        let _ = Self::new(fortanix_vme_abi::SERVER_PORT)
            .and_then(|mut client| client.send(&Request::Exit { code }));
    ...
    fn send(&mut self, req: &Request) -> Result<(), io::Error> {
        let req: Vec<u8> = serde_cbor::ser::to_vec(req)
            .map_err(|_e| io::Error::new(ErrorKind::Other, "serialization failed"))?;
    ...
    fn receive(&mut self) -> Result<Response, io::Error> {
        ...
        match resp {
            fortanix_vme_abi::Response::Failed(fortanix_vme_abi::Error::Command(kind)) => {
                Err(io::Error::from(io::ErrorKind::from(kind)))
            }
            fortanix_vme_abi::Response::Failed(fortanix_vme_abi::Error::SystemError(errno)) => {
