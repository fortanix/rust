...
use libc::{self, MSG_PEEK, c_int, c_void};
...
use crate::os::unix::prelude::{FromRawFd, IntoRawFd, RawFd};
use crate::sync::{Mutex, OnceLock};
use crate::sys::fd::FileDesc;
use crate::sys::fortanixvme::client::{Client, ConnectionInfo, Fortanixvme};
...
macro_rules! not_available {
    () => {
        return Err(io::const_error!(io::ErrorKind::Unsupported, &"Not available on Fortanixvme"))
    };
}
...
static INCOMING_INFO: OnceLock<Mutex<Vec<IncomingInfo>>> = OnceLock::new();
...
fn take_incoming_connection_info(runner_port: u32) -> Option<IncomingInfo> {
    ...
    info.iter()
        .enumerate()
        .find_map(|(idx, info)| if info.runner_port == runner_port { Some(idx) } else { None })
...
impl Socket {
    ...
    pub fn new_pair(fam: c_int, ty: c_int) -> io::Result<(Socket, Socket)> {
        unsafe {
            ...
            Ok((
                Socket::new(FileDesc::from_raw_fd(fds[0])),
                Socket::new(FileDesc::from_raw_fd(fds[1])),
            ))
        }
    }
    ...
    pub fn duplicate(&self) -> io::Result<Socket> {
        Ok(Socket { inner: self.inner.duplicate()? })
    }
    ...
    pub fn accept(
        ...
        unsafe {
            ...
            Ok(Socket { inner: FileDesc::from_raw_fd(fd) })
        }
    }
}
...
impl FromRawFd for Socket {
    unsafe fn from_raw_fd(raw_fd: RawFd) -> Self {
        Socket { inner: FromRawFd::from_raw_fd(raw_fd) }
    }
}
...
///
/// FortanixVME doesn't support DNS resolution but rather accepts hostnames in
...
/// This function is copied from sgx counterpart.
...
impl TcpStream {
    pub fn connect<A: ToSocketAddrs>(addr: A) -> io::Result<TcpStream> {
        each_addr(addr, |addr| {
            ...
            let fd = runner.open_proxy_connection(addr.to_owned())?;
            Ok(TcpStream { inner: Socket::new(fd) })
        })
    }
...
impl TcpListener {
    pub fn bind<A: ToSocketAddrs>(addr: A) -> io::Result<TcpListener> {
        each_addr(addr, |addr| {
            let mut runner = Client::new(fortanix_vme_abi::SERVER_PORT)?;
            let fd = runner.bind_socket(addr.to_owned())?;
            Ok(TcpListener { inner: Socket::new(fd) })
        })
    }
    ...
    pub fn accept(&self) -> io::Result<(TcpStream, SocketAddr)> {
        ...
        // FIXME: simplify solution by letting the runner start a vsock listener in (1) and
