cfg_select! {
    any(
        all(target_family = "unix", not(any(target_os = "l4re", all(target_arch = "x86_64", target_os = "linux", target_env = "fortanixvme")))),
        target_os = "windows",
        target_os = "hermit",
...
#[cfg_attr(
    ...
    not(any(all(target_os = "linux", not(target_env = "fortanixvme")), target_os = "windows")),
    allow(dead_code)
)]
