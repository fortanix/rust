...
// The reason why these tests are here rather than
// crate::fs::tests is, all of the fs tests must be
// disabled for fortanixvme but the following one.
// For this reason we exclude `fs/tests.rs` module completely
// and add fortanixvme fs test(s) here.
#[cfg(target_env = "fortanixvme")]
#[test]
fn test_fortanixvme_fs_access() {
    use crate::fs::File;
    use crate::io::ErrorKind;

    let allowed_files = ["/dev/sev-guest", "/dev/tdx-guest", "/dev/nsm"];
    for path in allowed_files {
        let res = File::open(path);
        if let Err(err) = res {
            assert!(err.kind() != ErrorKind::Unsupported);
        } else {
            // This would be the case if tests are running in an enclave
        }
    }

    // The following list can contain any files
    // except the one in allowed_files list.
    let other_files = ["/bin/sh", "/etc/passwd", "/tmp", "/var/run"];
