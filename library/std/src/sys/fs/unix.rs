...
static FORTANIXVME_ALLOWED_FILES: [&'static CStr; 3] =
    [c"/dev/nsm", c"/dev/sev-guest", c"/dev/tdx-guest"];
...
impl File {
    pub fn open(path: &Path, opts: &OpenOptions) -> io::Result<File> {
        run_path_with_cstr(path, &|path| {
            #[cfg(all(target_arch = "x86_64", target_os = "linux", target_env = "fortanixvme"))]
            {
                if FORTANIXVME_ALLOWED_FILES.contains(&path) {
                    File::open_c(path, opts)
                } else {
                    Err(Error::new(
                        io::ErrorKind::Unsupported,
                        "File accesses not supported on fortanixvme",
                    ))
                }
            }
