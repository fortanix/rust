...
fn main() {
    ...
    let mut elf =
        elf::ElfStream::<elf::endian::LittleEndian, _>::open_stream(File::open(f.path()).unwrap())
            .unwrap();
    ...
    // # Write the type
    let contents = format!(
        "#[repr(align({}), C)] pub struct Alloc {{ _0: [u8; {}] }}",
        sn_alloc_align, sn_alloc_size
    );
...
fn get_u64_at_symbol(sym: elf::symbol::Symbol, elf: &mut ElfStream<LittleEndian, File>) -> u64 {
    ...
    let (data, _) =
        elf.section_data(&elf.section_headers()[sym.st_shndx as usize].clone()).unwrap();
